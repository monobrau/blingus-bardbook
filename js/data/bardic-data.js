/**
 * Bardic inspiration data (curated revamp, best song-faithful parodies per type)
 */
window.BlingusData = window.BlingusData || {};

const bardic = {
  'Attack Rolls': [
    {t:"Another one bites the dust, now swing; let victory chorus sing.", s:"Another One Bites the Dust", a:"Queen"},
    {t:"Pour some sugar on me, sweeten the smack; bonus attack, no slack.", s:"Pour Some Sugar on Me", a:"Def Leppard"},
    {t:"Enter Sandman, tuck 'em in; lullaby swing with a wicked grin.", s:"Enter Sandman", a:"Metallica"},
    {t:"Sweet child o' mine, sweet crit o' time; line up the sight and shine.", s:"Sweet Child O' Mine", a:"Guns N' Roses"},
    {t:"Boom boom pow, extra oomph now; numbers bow, the swing lands now.", s:"Boom Boom Pow", a:"The Black Eyed Peas"},
    {t:"Thunderstruck? Nah, thunder-smacked; roll that boom, get impact.", s:"Thunderstruck", a:"AC/DC"},
    {t:"Hit me with your best shot, now I will; add the thrill, line up the kill.", s:"Hit Me With Your Best Shot", a:"Pat Benatar"},
    {t:"Shoot to thrill, load the die; bonus on the bull's-eye.", s:"Shoot to Thrill", a:"AC/DC"},
    {t:"Eye of the tiger, lock the sight; bonus bite, the swing lands right.", s:"Eye of the Tiger", a:"Survivor"},
    {t:"Mama said knock you out; bonus on the swing, that's what I'm talkin' about.", s:"Mama Said Knock You Out", a:"LL Cool J"},
    {t:"Jump around, jump around; add the swing, leave the ground.", s:"Jump Around", a:"House of Pain"},
  ],
  'Saving Throws': [
    {t:"Survivor, I'm a survivor; pass that save, come out a thriver, a survivor.", s:"Survivor", a:"Destiny's Child"},
    {t:"Stayin' alive, ah, ah, ah, ah; keep that beat, pass by harm.", s:"Stayin' Alive", a:"Bee Gees"},
    {t:"Under pressure, hold steady; breathe and pass, be ready.", s:"Under Pressure", a:"Queen & David Bowie"},
    {t:"Stronger, what doesn't kill you makes you stronger; save that danger, no longer.", s:"Stronger", a:"Kanye West"},
    {t:"I will survive, as long as I know how to save, I'll thrive.", s:"I Will Survive", a:"Gloria Gaynor"},
    {t:"Tubthumping, I get knocked down, then get up again; save that pain, win.", s:"Tubthumping", a:"Chumbawamba"},
    {t:"Don't fear the reaper, not today; more cowbell when we play.", s:"Don't Fear the Reaper", a:"Blue Öyster Cult"},
    {t:"It's my life, my save, my way; I hold the danger at bay.", s:"It's My Life", a:"Bon Jovi"},
    {t:"Hold on, just one more save; for one more round I'm brave.", s:"Hold On", a:"Wilson Phillips"},
    {t:"Regulate, I modulate; pass that save, now you're straight.", s:"Regulate", a:"Warren G & Nate Dogg"},
    {t:"This is how we do it; hold the line, the save goes through it.", s:"This Is How We Do It", a:"Montell Jordan"},
  ],
  'Skill - Persuasion/Performance/Deception': [
    {t:"Never gonna give you up, on this plan; trust the band, be our fan.", s:"Never Gonna Give You Up", a:"Rick Astley"},
    {t:"Sweet Caroline, good times never seemed so good… now nod; you should.", s:"Sweet Caroline", a:"Neil Diamond"},
    {t:"Personal Jesus, reach out, touch faith in this pitch; we'll make you rich.", s:"Personal Jesus", a:"Depeche Mode"},
    {t:"The way you make me feel, approve the appeal; seal the deal.", s:"The Way You Make Me Feel", a:"Michael Jackson"},
    {t:"Smooth operator, silver tongue, criminal charm; no alarm.", s:"Smooth Operator", a:"Sade"},
    {t:"Uptown funk you up, charm 'em up; smooth talk, seal it up.", s:"Uptown Funk", a:"Bruno Mars"},
    {t:"Respect, find out what it means; I sell the deal in between.", s:"Respect", a:"Aretha Franklin"},
    {t:"Don't stop me now, I'm having a good time; trust the pitch, trust the rhyme.", s:"Don't Stop Me Now", a:"Queen"},
    {t:"Take on me, take me on; sign right here, and now you're won.", s:"Take On Me", a:"a-ha"},
    {t:"No diggity, no doubt; they buy the pitch I just put out.", s:"No Diggity", a:"Blackstreet"},
    {t:"Gettin' jiggy with it; smile, nod, now commit.", s:"Gettin' Jiggy Wit It", a:"Will Smith"},
  ],
  'Skill - Stealth/Sleight/Blend In': [
    {t:"Every breath you take, no sound I make; watch me vanish in my wake.", s:"Every Breath You Take", a:"The Police"},
    {t:"Where the streets have no name, neither do my tracks; I leave no facts.", s:"Where the Streets Have No Name", a:"U2"},
    {t:"Enjoy the silence, words are needless; footprints, feckless.", s:"Enjoy the Silence", a:"Depeche Mode"},
    {t:"Smooth criminal, Annie, are you okay? They won't be; unseen spree.", s:"Smooth Criminal", a:"Michael Jackson"},
    {t:"Yesterday, all my noise seemed far away; now silence leads the way.", s:"Yesterday", a:"The Beatles"},
    {t:"Ghost, I'm a ghost down the hall; no creak, no sound, no trace at all.", s:"Ghost", a:"Justin Bieber"},
    {t:"The sound of silence, my footsteps gone; only shadows carry on.", s:"The Sound of Silence", a:"Simon & Garfunkel"},
    {t:"Bad guy, duh, but quiet; tip-toe past your private riot.", s:"Bad Guy", a:"Billie Eilish"},
    {t:"I walk the line, quiet feet, quiet mind; shadows signed.", s:"I Walk the Line", a:"Johnny Cash"},
    {t:"It's tricky to sneak a path like that; no sound, no trail, and that's a fact.", s:"It's Tricky", a:"Run-D.M.C."},
    {t:"Parents just don't understand; they never saw me slip the plan.", s:"Parents Just Don't Understand", a:"DJ Jazzy Jeff & The Fresh Prince"},
  ],
  'Skill - Investigation/Perception/Insight': [
    {t:"Sweet dreams are made of this, motives, means, and evidence; case in bliss.", s:"Sweet Dreams (Are Made of This)", a:"Eurythmics"},
    {t:"Behind blue eyes, I read the lie; the tell is in your nervous sigh.", s:"Behind Blue Eyes", a:"The Who"},
    {t:"I still haven't found what I'm looking for, oh wait, right here; clue crystal clear.", s:"I Still Haven't Found What I'm Looking For", a:"U2"},
    {t:"Every little thing she does is magic, and mildly suspicious; noted, officious.", s:"Every Little Thing She Does Is Magic", a:"The Police"},
    {t:"Under pressure, cracks reveal the seam; I thread the scheme.", s:"Under Pressure", a:"Queen & David Bowie"},
    {t:"Private eyes, they're watching you; I read the room, I spot the clue.", s:"Private Eyes", a:"Hall & Oates"},
    {t:"Changes, turn and face the strange; pattern shift within range.", s:"Changes", a:"David Bowie"},
    {t:"Somebody's watching me, and it's me; I clock the thing you didn't see.", s:"Somebody's Watching Me", a:"Rockwell"},
    {t:"I can see clearly now, the clue is found; truth unbound.", s:"I Can See Clearly Now", a:"Johnny Nash"},
    {t:"Informer, informer; I name the clue, that's the primer.", s:"Informer", a:"Snow"},
    {t:"Can I kick it? Yes you can; I read the room, that's the plan.", s:"Can I Kick It?", a:"A Tribe Called Quest"},
  ],
  'Skill - Athletics/Acrobatics/Climb/Run': [
    {t:"Jump around, up and down; I vault clean across the town, jump around.", s:"Jump Around", a:"House of Pain"},
    {t:"Footloose, kick off your Sunday shoes; I leap the gap, can't lose.", s:"Footloose", a:"Kenny Loggins"},
    {t:"I like to move it, move it; now you move it too; parkour debut.", s:"I Like to Move It", a:"Reel 2 Real"},
    {t:"Can't stop the feeling, feet start dealing; hop, skip, ceiling.", s:"Can't Stop the Feeling!", a:"Justin Timberlake"},
    {t:"Walk this way, run that way; hurdle clean, no delay.", s:"Walk This Way", a:"Aerosmith"},
    {t:"Back in black, track attack; sprint the line, don't look back.", s:"Back in Black", a:"AC/DC"},
    {t:"Highway to Hell, on ramp to speed; overtake with heed.", s:"Highway to Hell", a:"AC/DC"},
    {t:"Beat it, fatigue, beat it; second wind, repeat it.", s:"Beat It", a:"Michael Jackson"},
    {t:"Born to run, tramps like us; clear the hurdle, zero fuss.", s:"Born to Run", a:"Bruce Springsteen"},
    {t:"Bust a move; clear the gap; land the leap, that's the trap.", s:"Bust a Move", a:"Young MC"},
    {t:"Gettin' jiggy with it; vault the rail, now commit.", s:"Gettin' Jiggy Wit It", a:"Will Smith"},
  ],
  'Initiative': [
    {t:"You really got me, spooled and primed; I'm on time.", s:"You Really Got Me", a:"Van Halen"},
    {t:"Billie Jean, light on my feet; step to the front, bring heat.", s:"Billie Jean", a:"Michael Jackson"},
    {t:"Brass monkey, funky and spry; jump the line, get mine, here's why.", s:"Brass Monkey", a:"Beastie Boys"},
    {t:"Start me up, and never stop; first to pop, top of the crop.", s:"Start Me Up", a:"The Rolling Stones"},
    {t:"Gimme shelter, nah, give me turn one; battle's begun.", s:"Gimme Shelter", a:"The Rolling Stones"},
    {t:"Right now, I move first; I'm out the gate to quench the thirst.", s:"Right Now", a:"Van Halen"},
    {t:"Wake me up before you go-go, I'm already gone; blink and I'm on.", s:"Wake Me Up Before You Go-Go", a:"Wham!"},
    {t:"Ready or not, here I come; first in line, I outrun.", s:"Ready or Not", a:"The Fugees"},
    {t:"Don't stop me now, I'm a shooting star; first to act, I'm out far.", s:"Don't Stop Me Now", a:"Queen"},
    {t:"It's tricky to jump the line like that; I move first, and that's a fact.", s:"It's Tricky", a:"Run-D.M.C."},
    {t:"Whoomp, there it is; I go first, the fight is his.", s:"Whoomp! (There It Is)", a:"Tag Team"},
  ],
  'Cutting Words - Attack': [
    {t:"Bulletproof? Not them; I shave the swing, that's the hymn.", s:"Bulletproof", a:"La Roux"},
    {t:"Under pressure, I nick the aim; their swing goes lame.", s:"Under Pressure", a:"Queen & David Bowie"},
    {t:"You shook me all night long, but not this strike; I minus the like.", s:"You Shook Me All Night Long", a:"AC/DC"},
    {t:"Titanium? Shoot, they fall; I tax the swing, that's the call.", s:"Titanium", a:"David Guetta ft. Sia"},
    {t:"Iron man, armor's on; I still subtract, threat withdrawn.", s:"Iron Man", a:"Black Sabbath"},
    {t:"Hit me with your best shot, blocked; I spend the die, swing docked.", s:"Hit Me With Your Best Shot", a:"Pat Benatar"},
    {t:"I'm still standing, yeah, yeah, yeah; their attack denied, au contraire.", s:"I'm Still Standing", a:"Elton John"},
    {t:"U can't touch this, hammer time; I minus the swing, that's the crime.", s:"U Can't Touch This", a:"MC Hammer"},
    {t:"Stop! In the name of love, nope, not this blow; I tax the throw.", s:"Stop! In the Name of Love", a:"The Supremes"},
    {t:"Regulate, I modulate; I shave the attack, now they're straight.", s:"Regulate", a:"Warren G & Nate Dogg"},
    {t:"Check yo self before you wreck yo self; that swing's back on the shelf.", s:"Check Yo Self", a:"Ice Cube"},
  ],
  'Cutting Words - Check': [
    {t:"Smooth operator? Not this roll; I shave the check, take the toll.", s:"Smooth Operator", a:"Sade"},
    {t:"It's tricky to pass a check like that; I minus the try, and that's a fact.", s:"It's Tricky", a:"Run-D.M.C."},
    {t:"Every breath you take, I tax the sneak; their quiet plan springs a leak.", s:"Every Breath You Take", a:"The Police"},
    {t:"Informer, you no say; I nick the check, that's the play.", s:"Informer", a:"Snow"},
    {t:"Can I kick it? No you can't; I spend the die, you miss the chant.", s:"Can I Kick It?", a:"A Tribe Called Quest"},
    {t:"Private eyes, they're slipping too; I minus the look, no clue for you.", s:"Private Eyes", a:"Hall & Oates"},
    {t:"Bad guy, duh, but loud; I dock the stealth, no vanishing crowd.", s:"Bad Guy", a:"Billie Eilish"},
    {t:"Respect, find out what it means; I shave the pitch, ruin the scene.", s:"Respect", a:"Aretha Franklin"},
    {t:"No diggity, no doubt; I tax that check you just put out.", s:"No Diggity", a:"Blackstreet"},
    {t:"Parents just don't understand; I minus the slip, that's the plan.", s:"Parents Just Don't Understand", a:"DJ Jazzy Jeff & The Fresh Prince"},
    {t:"Somebody's watching me, and it's me; I dock the notice they didn't see.", s:"Somebody's Watching Me", a:"Rockwell"},
  ],
  'Cutting Words - Damage': [
    {t:"Another one bites, I steal the thrust; minus that gust.", s:"Another One Bites the Dust", a:"Queen"},
    {t:"Pour some sugar on me? I unsweet the smack; damage back.", s:"Pour Some Sugar on Me", a:"Def Leppard"},
    {t:"Thunderstruck, I amp it down; roll the boom, I steal the crown.", s:"Thunderstruck", a:"AC/DC"},
    {t:"Hells bells, ring the hit; I tax the damage, never quit.", s:"Hells Bells", a:"AC/DC"},
    {t:"Crazy train, off the rails; I minus the pain that derails.", s:"Crazy Train", a:"Ozzy Osbourne"},
    {t:"Shot through the heart, and you're to blame; I shave the pain, write my name.", s:"You Give Love a Bad Name", a:"Bon Jovi"},
    {t:"Boom boom pow, extra oomph? Nope; I dock the numbers, that's the trope.", s:"Boom Boom Pow", a:"The Black Eyed Peas"},
    {t:"Killing in the name, of minus flame; I tax the damage game.", s:"Killing in the Name", a:"Rage Against the Machine"},
    {t:"T.N.T., I'm dynamite; I steal the boom before it ignites.", s:"T.N.T.", a:"AC/DC"},
    {t:"Mama said knock you out; I minus the hurt, that's what I'm talkin' about.", s:"Mama Said Knock You Out", a:"LL Cool J"},
    {t:"Whoomp, there it is; less pain, they feel the fizz.", s:"Whoomp! (There It Is)", a:"Tag Team"},
  ],
};

window.BlingusData.bardic = bardic;
